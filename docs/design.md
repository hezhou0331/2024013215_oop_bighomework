...
- 异常捕获与转换发生在此层：模型异常 → `RES` 状态码；错误详情 → `LastError` 成员。
...
`ValidationInfo`、`ScheduleInfo`、`StatisticsInfo`，全部采用公有数据成员。
这一设计选择基于以下理由：

**纯数据结构，无业务规则**  
DTO 的唯一职责是承载数据——将模型层计算结果传递给表现层。这些类既不维护
任何不变量，也不强制任何读写约束。数据校验与业务逻辑全部由模型层负责
（Task、Dependency、Resource、ProjectValidator、CPMScheduler 等），
而非 DTO。因此 DTO 没有理由采用 getter/setter 包装。

**编码规范明确的例外条款**  
编码规范 2.8.9 条例外 1 指出："纯数据容器可采用公有数据成员，但必须显式
定义构造、拷贝构造、拷贝赋值与析构。"本项目每个 DTO 都严格遵守此条款：
显式声明所有四个特殊成员函数，不使用编译器生成的默认实现。

**界面与控制器的契约内聚**  
DTO 内嵌于 ProjectController 类（规范 2.8.13），使数据交换协议在一个头文件中
完整定义。接收端（ConsoleUI）导入 ProjectController.hpp 后，一目了然地看到
能获得哪些数据字段，无需查阅散落各处的 getter 名单。

**避免虚假复杂性**  
若将 6 个 DTO 共 30+ 个公有数据成员改写为 getter/setter，会增加约 60 行代码，
提高代码 I/O（含编译时间），但不带来逻辑上的安全性——数据仍然来自模型层的
计算结果，消费者（ConsoleUI）仍然是只读使用。这是"虚假的防护"。

**与工业实践一致**  
在 MVC 框架、REST API 响应序列化、传输层数据结构中，DTO 与消息格式（JSON、
Protocol Buffer）的惯例都是公有字段。本项目采取一致的设计哲学。

综上，DTO 的公有成员是"功能合适、规范合规、复杂度合理"的设计，不是偷懒。
